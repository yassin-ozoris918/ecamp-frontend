...
export function AIQuestionReviewStudio({ examId, initialQuestions, onSaved, onCancel }: AIQuestionReviewStudioProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-neutral-950/80 backdrop-blur-sm flex items-center justify-center p-6">
      <div className="bg-neutral-900 border border-white/[0.08] w-full max-w-6xl max-h-full rounded-3xl shadow-2xl flex flex-col">
        <div className="p-6 border-b border-white/[0.08] flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-neutral-400 mt-1">Review, tweak, and finalize AI-generated questions before committing them to the exam.</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
            ...
            <div key={idx} className="bg-white/[0.02] border border-white/[0.04] rounded-2xl p-5 relative group">
              ...
              <div className="grid grid-cols-12 gap-6">
                <div className="col-span-8 space-y-4">
                  <div>
                    <label className="text-xs font-bold text-neutral-400 uppercase tracking-wider mb-2 block">Content Text / Prompt</label>
                    <textarea 
                  ...
                  {['MCQ', 'TRUE_FALSE'].includes(q.type) && (
                    <div className="space-y-3">
                      <label className="text-xs font-bold text-neutral-400 uppercase tracking-wider block">Options & Correct Answer</label>
                  ...
                  {['ESSAY', 'SHORT_ANSWER'].includes(q.type) && (
                    <div>
                      <label className="text-xs font-bold text-neutral-400 uppercase tracking-wider mb-2 block">Reference Rubric (AI grading baseline)</label>
                      <textarea 
                ...
                <div className="col-span-4 space-y-4 border-l border-white/[0.04] pl-6">
                  ...
                  <div>
                    <label className="text-xs font-bold text-neutral-400 uppercase tracking-wider mb-2 block">Points Weight</label>
                    <input 
                      type="number"
                      className="input-field"
                      value={q.points}
          ...
          {questions.length === 0 && (
             <div className="text-center text-neutral-500 py-12">No questions extracted. Please try another file.</div>
          )}
        </div>
      </div>
    </div>
