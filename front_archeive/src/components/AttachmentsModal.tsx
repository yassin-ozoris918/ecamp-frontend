...
export function AttachmentsModal({ isOpen, onClose, targetId, targetType, title }: AttachmentsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-2xl bg-[#0B0C10] border border-white/10 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-display font-bold text-white">Manage Attachments</h2>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <form onSubmit={handleUploadSubmit} className="bg-neutral-900/50 p-4 rounded-xl border border-neutral-800 space-y-4">
            <div>
              <label className="block text-xs font-medium text-neutral-400 mb-2">Select File</label>
              <input
                type="file"
            ...
            {selectedFile && (
              <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                <label className="block text-xs font-medium text-neutral-400 mb-2">Display Title</label>
          ...
          {/* Attachments List */}
          <div>
            <h3 className="text-sm font-semibold text-neutral-300 mb-4">Uploaded Files</h3>
            {isLoading ? (
              ...
              <div className="text-center py-8 bg-neutral-900/30 rounded-xl border border-neutral-800/50 border-dashed">
                ...
                <p className="text-sm text-neutral-500">No attachments uploaded yet</p>
              </div>
            ) : (
                ...
                {attachments.map((att) => (
                  <div key={att.id} className="flex items-center justify-between p-3 bg-neutral-900/50 rounded-xl border border-neutral-800 hover:border-neutral-700 transition-colors group">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <a href={att.fileUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-white hover:text-cyan-400 transition-colors">
                          {att.title}
                        </a>
                      </div>
                    </div>
                    <button
                      ...
                      className="p-2 text-neutral-500 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100 disabled:opacity-50"
