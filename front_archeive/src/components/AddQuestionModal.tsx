      ...
      <div className="w-full max-w-lg rounded-2xl border border-neutral-800 bg-neutral-900 p-6 text-white shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="space-y-2">
            <label className="block text-xs text-neutral-400">Answer Options (Select correct radio link)</label>
            {options.map((opt, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                {options.length > 2 && (
                  <button type="button" onClick={() => removeOptionRow(idx)} className="text-xs text-neutral-500 hover:text-red-400">&times;</button>
                )}
              </div>
            ))}
          ...
          <div>
            <label className="block text-xs text-neutral-400 mb-1">Points Allocated</label>
          ...
          <div className="flex justify-end gap-2 border-t border-neutral-800 pt-3">
            <button type="button" onClick={onClose} className="px-3 py-2 text-xs text-neutral-400">Cancel</button>
